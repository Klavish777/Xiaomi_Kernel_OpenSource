...
## Animated agent dashboard

- Overview places the technical-analysis brain, internet-reference Earth and Paper/MT5 miner controls above the price chart. The balance is centered over a realistic Earth illustration. Tapping each mascot toggles its corresponding agent; adjacent settings buttons open the agent controls. The brain and miner have idle/active motion, the Earth floats with an animated orbit, and reduced-motion preferences are respected.
...
- Bridge-side hard limits remain: maximum 0.01 lot, no more than one agent position, mandatory stop-loss of at least 20 pips and take-profit of at least 30 pips, maximum 5-pip spread, and a 1% account-wide daily-loss stop. Each agent also latches off at 80,000,000 units of its account currency: Paper uses CAD virtual equity, while MT5 uses the greater of account balance or equity without currency conversion. The MT5 latch is keyed to the login/server account. At the cap, the agent closes only positions it owns; manual/unrelated positions are not closed. The Paper agent closes its own virtual position and cannot be restarted after its goal is reached. A cash-profit close target scales at 30 account-currency units per 1.00 lot: 0.30 at 0.01 lot and 0.60 at 0.02 lot; **new orders remain capped at 0.01 lot**, so the 0.02 figure is only the scaling formula unless the safety cap is explicitly changed in a future version. Profit target is not guaranteed realized profit: server SL/TP, daily-loss protection, schedule and signal exits remain active and can close at a loss; gaps/slippage can also worsen fills. Two consecutive losing agent trades trigger a one-hour entry cooldown. At least five recent closed trades with win rate below 40% tightens the RSI entry filter. Limits fail closed if the broker's minimum lot exceeds 0.01, quotes are stale, algorithmic trading is disabled, or the broker does not allow both directions. Pausing the runner stops new entries and strategy exits; any already-open broker position remains at MT5 with server-side SL/TP until a stop/target fills or the user closes it.
...
Bybit MT5 is supported through its dedicated **MT5 CFD Account** (not Bybit's Unified Trading Account): create/activate that account on Bybit, then enter its MT5 account ID, MT5 trading password and exact assigned server in Money Work. The connection form offers Bybit Live/Demo server suggestions, but the server shown in your Bybit account is authoritative. Never use your Bybit website password or send credentials in chat. Deposits/transfers between Bybit accounts remain on Bybit; Money Work does not transfer funds.
...
`npm test` runs JavaScript analyst-consensus/reference freshness tests and Python unit tests covering volume/risk policy and mocked broker execution, including live confirmation, pip and cash-profit targets, the 80-million account-currency stop, Paper equity-stop behavior, stale prices, manual-position conflicts, spread limits, daily loss stops, reference-gate behavior, position closure without a reference, and adaptive cooldowns. These tests do not connect to a personal brokerage account.
